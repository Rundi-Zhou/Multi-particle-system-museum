...
class CPartSys {
    ...
    initBouncyBall(partCount, forces, walls){
        ...
        for (var i = 0, j = 0; i < this.partCount; i++, j += PART_MAXVAR){  // init mass
            ...
            this.S0[j + PART_XPOS] = 0.5 * (Math.random()-0.5);
            this.S0[j + PART_YPOS] = 0.5 * (Math.random()-0.5);
            this.S0[j + PART_ZPOS] = 0;
            this.S0[j + PART_WPOS] = 1.0;
            this.S0[j + PART_XVEL] = Math.random()-0.5;
            this.S0[j + PART_YVEL] = Math.random()-0.5;
    ...
    initSpringSnake(partCount, forces, walls){
        ...
        for (var i = 0, j = 0; i < this.partCount; i++, j += PART_MAXVAR){
            ...
            this.S0[j + PART_XPOS] = 0.01*i;
            this.S0[j + PART_YPOS] = 0.2-0.01*i;
            this.S0[j + PART_ZPOS] = 0.02*i;
            this.S0[j + PART_WPOS] =  1.0;
            ...
            switch(F[j].forceType){
                ...
                case F_SPRING_SNAKE:  // Spring snake
                    ...
                    if (isFixed){ 
                        ...
                        S[PART_Y_FTOT] = 0;
                        if(isgravOn){
                            S[PART_Z_FTOT] = S[i * PART_MAXVAR + PART_MASS] * F[j].gravConst;
                        }
                        else{
                            S[PART_Z_FTOT] = 0;
                        }
                ...
                case F_FLOCK2:
                    ...
                    for (var i = 0; i < this.partCount; i++){        
                        ...
                        var dis = distance3D([0,0,1], S.slice(i*PART_MAXVAR + PART_XPOS, i*PART_MAXVAR + PART_ZPOS + 1));
                        if (dis > pBallRadius + ballRadius){
                            S[i*PART_MAXVAR + PART_X_FTOT] += 0.5*(0 - S[i*PART_MAXVAR + PART_XPOS]);
                            S[i*PART_MAXVAR + PART_Y_FTOT] += 0.5*(0 - S[i*PART_MAXVAR + PART_YPOS]);
                            S[i*PART_MAXVAR + PART_Z_FTOT] += 0.5*(1 - S[i*PART_MAXVAR + PART_ZPOS]);
                        }
    ...
    doConstraints(S1, S0, W){
        ...
        for (var j = 0; j < this.wallCount; j++){
            ...
            switch(W[j].wallType){
                ...
                case WTYPE_CONTAINER_BALL:
                    for (var i = 0, k = 0; i < this.partCount; i++, k += PART_MAXVAR){
                        if (Math.sqrt(S1[i * PART_MAXVAR + PART_XPOS]*S1[i * PART_MAXVAR + PART_XPOS]+S1[i * PART_MAXVAR + PART_YPOS]*S1[i * PART_MAXVAR + PART_YPOS]+S1[i * PART_MAXVAR + PART_ZPOS]*S1[i * PART_MAXVAR + PART_ZPOS]) > 1.0 - ballRadius){
                            S1[k + PART_XPOS] = S0[k + PART_XPOS];
                            S1[k + PART_YPOS] = S0[k + PART_YPOS];
                            S1[k + PART_ZPOS] = S0[k + PART_ZPOS];
                ...
                case WTYPE_CYLENDER:
                    for (var i = 0, k = 0; i < this.partCount; i++, k += PART_MAXVAR){
                        if (Math.sqrt(S1[i * PART_MAXVAR + PART_XPOS]*S1[i * PART_MAXVAR + PART_XPOS]+S1[i * PART_MAXVAR + PART_YPOS]*S1[i * PART_MAXVAR + PART_YPOS]) > 1.0 - ballRadius){
                            S1[k + PART_XPOS] = S0[k + PART_XPOS];
                            S1[k + PART_YPOS] = S0[k + PART_YPOS];
                            S1[k + PART_ZPOS] = S0[k + PART_ZPOS];
                ...
                case WTYPE_TORNADO2:
                    for (var i = 0, k = 0; i < this.partCount; i++, k += PART_MAXVAR){
                        ...
                        if (S1[k + PART_AGE] < 0 ){  // Dead
                            ...
                            S1[k + PART_XPOS] = 2 * (Math.random()-0.5);
                            S1[k + PART_YPOS] = 2 * (Math.random()-0.5);
                            while(S1[k + PART_XPOS]*S1[k + PART_XPOS]+S1[k + PART_YPOS]*S1[k + PART_YPOS]>1){
                                S1[k + PART_XPOS] = 2 * (Math.random()-0.5);
                                S1[k + PART_YPOS] = 2 * (Math.random()-0.5);
                            }
                            S1[k + PART_ZPOS] = 0;
