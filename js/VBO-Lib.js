...
function VBObox1(){
    ...
    var totalSize = gndVerts.length + axisVerts.length 
                    + cubeVerts.length + sphVerts.length + cylVerts.length;
...
VBObox1.prototype.draw = function() {
    ...
    this.ModelMat = popMatrix();
    pushMatrix(this.ModelMat);
    this.ModelMat.translate(4.0, 0.0, 0.0);
    drawCube(this.ModelMat, this.u_ModelMatLoc);
    ...
    this.ModelMat = popMatrix();
    pushMatrix(this.ModelMat);
	drawSphere2(this.ModelMat, this.u_ModelMatLoc);
    ...
    this.ModelMat = popMatrix();
    pushMatrix(this.ModelMat);
	drawCylinder(this.ModelMat, this.u_ModelMatLoc);
}
...
function VBObox11(){
    ...
    'void main() {\n' +
    '   gl_PointSize = 20.0;\n' + 
    '   gl_Position = u_ModelMatrix * (a_Position + vec4(2.0,0.0,1.0,0.0)); \n' +
    '	v_Color = a_Color; \n' +	
    '} \n';
    ...
    this.partCount = 10;
    this.forces = [F_NONE, F_SPRING_SNAKE];
    this.walls = [WTYPE_CONTAINER_BALL];
    this.pSys.initSpringSnake(this.partCount, this.forces, this.walls);
    ...
    console.assert(PART_MAXVAR * this.FSIZE == this.vboStride, 
        "Uh oh! VBObox11.vboStride disagrees with attribute-size values!");
...
VBObox11.prototype.init = function(){
    this.shaderLoc = createProgram(gl, this.VERT_SRC, this.FRAG_SRC);
	if (!this.shaderLoc) {
        console.log(this.constructor.name + 
                                '.init() failed to create executable Shaders on the GPU. Bye!');
...
VBObox11.prototype.switchToMe = function() {
    gl.useProgram(this.shaderLoc);
    gl.bindBuffer(gl.ARRAY_BUFFER, this.vboLoc);
    gl.vertexAttribPointer( this.a_PosLoc,
                            this.vboFcount_a_Pos,
...
VBObox11.prototype.adjust = function() {
    if(this.isReady()==false) {
        console.log('ERROR! before' + this.constructor.name + 
                            '.adjust() call you needed to call this.switchToMe()!!');
    }
...
VBObox11.prototype.draw = function() {
    if (runMode > 1){
        if (runMode == 2) runMode = 1;  // do one step
...
VBObox11.prototype.isReady = function() {
    var isOK = true;
...
function VBObox12(){
    ...
    'void main() {\n' +
    '   gl_PointSize = 8.0;\n' + 
    '   gl_Position = u_ModelMatrix * (a_Position + vec4(-2.0 , 0.0 , 0.0 , 0.0)); \n' +
    '	v_Color = a_Color; \n' +	
    '} \n';
    ...
    this.partCount = 400;
    this.forces = [F_NONE, F_GRAV_E, F_DRAG];
    this.walls = [WTYPE_GROUND, WTYPE_CYLENDER,WTYPE_ZWALL_HI];
    this.pSys.initBouncyBall(this.partCount, this.forces, this.walls);
    ...
    console.assert(PART_MAXVAR * this.FSIZE == this.vboStride, 
        "Uh oh! VBObox12.vboStride disagrees with attribute-size values!");
...
VBObox12.prototype.init = function(){
    this.shaderLoc = createProgram(gl, this.VERT_SRC, this.FRAG_SRC);
	if (!this.shaderLoc) {
        console.log(this.constructor.name + 
                                '.init() failed to create executable Shaders on the GPU. Bye!');
...
VBObox12.prototype.switchToMe = function() {
    gl.useProgram(this.shaderLoc);
    gl.bindBuffer(gl.ARRAY_BUFFER, this.vboLoc);
    gl.vertexAttribPointer( this.a_PosLoc,
                            this.vboFcount_a_Pos,
...
VBObox12.prototype.adjust = function() {
    if(this.isReady()==false) {
        console.log('ERROR! before' + this.constructor.name + 
                            '.adjust() call you needed to call this.switchToMe()!!');
    }
...
VBObox12.prototype.draw = function() {
    if (runMode > 1){
        if (runMode == 2) runMode = 1;  // do one step
         // 1) DotFinder(): Find s0Dot from s0 & f0. 
         this.pSys.applyAllForces(this.pSys.S0, this.pSys.F0);
...
VBObox12.prototype.isReady = function() {
    var isOK = true;
...
function VBObox13(){
    ...
    'void main() {\n' +
    '   gl_PointSize = 8.0;\n' + 
    '   gl_Position = u_ModelMatrix * (a_Position+vec4(4.0 , 0.0 , 0.0 , 0.0)); \n' +
    '	v_Color = a_Color; \n' +	
    '} \n';
    ...
    this.partCount = 600;
    this.forces = [F_NONE, F_GRAV_E, F_DRAG];
    this.walls = [WTYPE_GROUND, WTYPE_XWALL_LO, WTYPE_XWALL_HI, WTYPE_YWALL_LO, WTYPE_YWALL_HI,  WTYPE_ZWALL_LO, WTYPE_ZWALL_HI];
    this.pSys.initBouncyBall(this.partCount, this.forces, this.walls);
    ...
    console.assert(PART_MAXVAR * this.FSIZE == this.vboStride, 
        "Uh oh! VBObox13.vboStride disagrees with attribute-size values!");
...
VBObox13.prototype.init = function(){
    this.shaderLoc = createProgram(gl, this.VERT_SRC, this.FRAG_SRC);
	if (!this.shaderLoc) {
        console.log(this.constructor.name + 
                                '.init() failed to create executable Shaders on the GPU. Bye!');
...
VBObox13.prototype.switchToMe = function() {
    gl.useProgram(this.shaderLoc);
    gl.bindBuffer(gl.ARRAY_BUFFER, this.vboLoc);
    gl.vertexAttribPointer( this.a_PosLoc,
                            this.vboFcount_a_Pos,
...
VBObox13.prototype.adjust = function() {
    if(this.isReady()==false) {
        console.log('ERROR! before' + this.constructor.name + 
                            '.adjust() call you needed to call this.switchToMe()!!');
    }
...
VBObox13.prototype.draw = function() {
    if (runMode > 1){
        if (runMode == 2) runMode = 1;  // do one step
         // 1) DotFinder(): Find s0Dot from s0 & f0. 
         this.pSys.applyAllForces(this.pSys.S0, this.pSys.F0);
...
VBObox13.prototype.isReady = function() {
    var isOK = true;
...
function VBObox14(){
    ...
    this.partCount = 600;
    this.forces = [F_NONE, F_GRAV_E, F_DRAG];
    this.walls = [WTYPE_FIRE, WTYPE_GROUND, WTYPE_XWALL_LO, WTYPE_XWALL_HI, WTYPE_YWALL_LO, WTYPE_YWALL_HI,  WTYPE_ZWALL_LO, WTYPE_ZWALL_HI];
    this.pSys.initBouncyBall(this.partCount, this.forces, this.walls);
    ...
    console.assert(PART_MAXVAR * this.FSIZE == this.vboStride, 
        "Uh oh! VBObox14.vboStride disagrees with attribute-size values!");
...
VBObox14.prototype.init = function(){
    this.shaderLoc = createProgram(gl, this.VERT_SRC, this.FRAG_SRC);
	if (!this.shaderLoc) {
        console.log(this.constructor.name + 
                                '.init() failed to create executable Shaders on the GPU. Bye!');
...
VBObox14.prototype.switchToMe = function() {
...
VBObox14.prototype.adjust = function() {
    if(this.isReady()==false) {
        console.log('ERROR! before' + this.constructor.name + 
                            '.adjust() call you needed to call this.switchToMe()!!');
    }
...
VBObox14.prototype.draw = function() {
    if (runMode > 1){
        if (runMode == 2) runMode = 1;  // do one step
        // 1) DotFinder(): Find s0Dot from s0 & f0. 
        this.pSys.applyAllForces(this.pSys.S0, this.pSys.F0);
...
VBObox14.prototype.isReady = function() {
    var isOK = true;
